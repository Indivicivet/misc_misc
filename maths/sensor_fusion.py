...
class SimulationResult:
    ...
    x_kf: np.ndarray
    x_cf: np.ndarray
    theoretical_kf_pos_var: list
    theoretical_cf_pos_var: list
    kf_errors: np.ndarray
    cf_errors: np.ndarray
    empirical_kf_var: np.ndarray
    empirical_cf_var: np.ndarray
    dt: float
    n_steps: int
    total_steps: int
...
def run_simulation(
    dt=0.01, n_steps=10, sigma_a=0.5, sigma_p=2.0, num_cycles=1000
) -> SimulationResult:
    # System matrices
    ...
    return SimulationResult(
        ...
        x_kf=x_kf,
        x_cf=x_cf,
        theoretical_kf_pos_var=theoretical_kf_pos_var,
        theoretical_cf_pos_var=theoretical_cf_pos_var,
        kf_errors=kf_errors,
        cf_errors=cf_errors,
        empirical_kf_var=empirical_kf_var,
        empirical_cf_var=empirical_cf_var,
        dt=dt,
        n_steps=n_steps,
...
if __name__ == "__main__":
    ...
    plt.plot(res.t, res.x_kf[0], "b-", label="Kalman Filter")
    plt.plot(res.t, res.x_cf[0], "g-", label="Complementary Filter")
    ...
    plt.hist(
        res.kf_errors[50:, res.n_steps - 2],
        bins=30,
        alpha=0.5,
        color="b",
        label="Kalman Filter",
    ...
    plt.hist(
        res.cf_errors[50:, res.n_steps - 2],
        bins=30,
        alpha=0.5,
        color="r",
        label="Complementary Filter",
    ...
    kf_error_diff = np.max(np.abs(res.empirical_kf_var - res.theoretical_kf_pos_var))
    print(
